...
\begin{description}[style=nextline]
  ...
  \item[L - Figure 3: This is a difficult image, I think. I was wondering whether removing the brown lines (since you only refer to the red ones through the text) would make it more clear. I have the feeling that the brown lines might be hiding some of the information about the red lines. Otherwise maybe replace the brown lines by just dots on the entrance point of the brown lines into the TPC, so we can still see the red lines we guess on the right? ]

  I am not sure, the message we want to convey here is that not all the cosmic rays that go through the MuCS go also through the TPC. Unfortunately, since the lines here are very dense replacing the solid lines with dotted lines will not make a lot of difference. This is a picture generated by the ROOT 3D display so I have limited options but I can see if we can make it clearer.

  \item[L - Figure 3: question! I don’t know if you have this number, maybe its interesting to know the fraction of red over total (brown+red) in this figure, i.e. the fraction of muons crossing the muon counter you expect to miss the TPC  in each configuration.]

  For the central and upstream configuration the fraction is above 99\%, while for the downstream is 62\%.

  \item[L - L168: A question I have here is, do you use the track objects from pandoraNu or fitted tracks from the Kalman filter which runs on the Pandora output tracks? If the latter, I think you need to mention that there is a further characterisation or fit of the track objects the Pandora pattern recognition is providing. If the former, do you use the track start/end point or the 3D space points? Also, I see that you are using the SDK paper as reference only, you could as well reference the Pandora public note as it contains descriptions more specific for MicroBooNE and the algorithms used in its reconstruction, I am not sure what the policy towards referencing public notes is, I think Matt and Michele would advise on this. ]

  We are using the output of pandoraCosmic, taking the start point. The policy is not to quote the public notes if possible, but maybe I can quote the Pandora paper that has been submitted to JINST?

  \item[L - Eq. 3.2: So here $y_{MuCS}$ is always constant, right? I was wondering whether it would make sense as a refinement for future analyses to extrapolate not only to the top of the TPC but to the y position of the TPC tracks you are comparing to (which would mean handling several extrapolated tracks to different y positions for the same event). In this way I think your purity could improve in regions where there are gaps (and maybe the systematic effect of gaps, still there as they affect in general the reconstructions of tracks, could hopefully be reduced). Might not make much difference though, your purity is quite high already. I was also wondering, since you have the information about the extrapolated entrance angles, is that something you could use as well in your extrapolated-TPC track matching? ]

  I don’t think that extrapolating to the y position of each TPC track will make a difference in terms of purity. It is true that it can happen that the MuCS cosmic ray starts in a region with missing wires and it is reconstructed below $y_{top}$, but this is also true for a random cosmic ray that happens to be close to the extrapolated point. We studied the effect of introducing an angular cut, but we found it to be negligible. You can find the plots in the Appendix A of the internal note.

  \item[L - Figure 4: I think the red arrow for the distance makes it a bit confusing, as it is not the distance you are actually using (which is in 3D), I would just remove the distance symbol in the image. What is the red line on the left TPC wall?  ]

  It is true that the distance is in 3D, but being a 2D projection I think it’s fine. The red line on the left correspond to the position of the MuCS flash but since we are not using the flash information anywhere I am not mentioning it. Maybe I could explain what it is in the caption?

  \item[L - Figure 6: for curiosity, is there a reason for the different binning in d (5-5-5-5-3-2-2-3-10 cm)? Is that so it provides equal number of events per bin?]

Not really, we just made more measurements in the region where P/A was closer to 1.

\item[L - Figure 7: I was wondering whether it would be possible to add a color scale with different shades of grey (!) proportional to the reconstruction efficiency; when printed (I am using a color printer) the upper and left sides are kind of “shadowed" making it difficult to see differences across the bins. ]
